...
If KW&"" <> "" Then
 ...
 ElseIf KT="kCode" Then
   'sqlK = sqlK & " AND ( ParCode LIKE '"&KW&"%' ) " 
 Else 'kName
   'sqlK = sqlK & " AND ( ParName LIKE '%"&KW&"%' ) " 
   sqlK = sqlK & " AND ( LogAUser LIKE '"&KW&"%' ) " 
 End If
End If
...
If yAct = "del_now" Then
 If KW<>"" Then
  sql = " DELETE FROM [TradePara] WHERE ParFlag='"&ModID&"' "&sqlK
...
ElseIf yAct = "del_sel" Then
 If sID<>"" Then
  sID = sID&"|"
  sID = Replace(sID,"',|","'")
  sql = " DELETE FROM [TradePara] WHERE ParCode IN ("&sID&") "
  Call rs_DoSql(conn,sql)
  Msg = "删除成功!"
 End If
Else
...
ParCode = ""
  ...
  sql = " SELECT * FROM [TradePara] WHERE 1=1 "&sqlK& " ORDER BY ParCode DESC " 
  'Response.Write sql 'ParFlag='"&ModID&"'
...
<table width="720" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#CCCCCC">
  <tr align="center" bgcolor="E0E0E0">
    <td height="21" colspan="6" align="center" bgcolor="f8f8f8"><table width="100%" border="0" cellpadding="1" cellspacing="0">
        <tr align="center">
          <td width="30%" rowspan="2" align="center"><strong>[模版设置]</strong> 列表</td>
  ...
  <tr align="center" bgcolor="#FFFFFF">
    ...
    <td align="center" bgcolor="#e8e8e8">Content</td>
...
<form name="flist" method="post" action="?">
  ...
  i = i + 1
		...
		ParCode = rs("ParCode")
		ParFlag = rs("ParFlag")
		ParRem = Show_Text(Left(rs("ParRem"),24))
		ParRem = Replace(ParRem,"<br>"," ")
    ...
    <tr bgcolor="<%=col%>">
      <td align="right"><%=i%>
      <input 
			name="yID" type="checkbox" id="yID" value="<%=ParCode%>"></td>
      <td><%=ParCode%></td>
      <td><%=ParFlag%>      </td>
      <td valign="top"><%=ParRem%></td>
