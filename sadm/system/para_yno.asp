...
sql = sql& ",ParText = '" & RequestS("ParText",3,96) &"' " 
...
<table width="640" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="e0e0e0">
  <tr align="center" bgcolor="f8f8f8">
    <td colspan="7"><table width="100%"  border="0" cellspacing="1" cellpadding="1">
        <tr>
          <td width="20%" align="center" nowrap><strong>开关参数</strong><br>
...
%>
  <form name="fmedit<%=i%>" method="post" action="?">
    <tr bgcolor="<%=col%>">
      ...
      <td colspan="2"><select name="ParText" id="ParText">
          <option value="X" <%If ParText="X" Then Response.Write("selected")%>>[默认]</option>
          <option value="Y" <%If ParText="Y" Then Response.Write("selected")%>>Y.是</option>
          <option value="N" <%If ParText="N" Then Response.Write("selected")%>>N.否</option>
        </select></td>
      ...
      <td align="center" bgcolor="<%=col%>"><input type="button" name="Button" value="删除" 
			onClick="Del_YN('?send=del&ParCode=<%=Show_jsStr(ParCode)%>','<%=Show_jsStr(ParName)%>')"></td>
    </tr>
  </form>
  <% 
rs.MoveNext 
...
%>
  ...
  <form name="fm01" method="post" action="?">
    <tr bgcolor="f0f0f0">
      ...
      <td><select name="ParText" id="ParText">
          <option value="X">[默认]</option>
