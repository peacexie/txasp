...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
  ...
  <%
iCont = 0 : sList = ""
If tmpList="Cont" Then
  sql = "SELECT KeyID,InfCont FROM "&ModTab&" "&sqlK
  If DP&"" <> "" Then
    sql = sql & " AND ( InfTyp2='"&DP&"' ) " 
  End If
  sql = sql& " ORDER BY LogATime ASC" ': Response.Write sql
...
ElseIf tmpList="Next" Then
   sql="SELECT KeyID,InfSubj,InfCont FROM "&ModTab&" "&sqlK
   If DP&"" <> "" Then
    sql = sql & " AND ( InfTyp2='"&DP&"' ) " 
   End If
   ...
   Do While Not rs.EOF
	 ...
	 ElseIf ID=KeyID Then
       ...
       InfSBak = Show_Text(InfSubj)
	 End If
     t="<span class='ItemAbout'><A href='?KeyID="&KeyID&"&ModID="&MD&"&TypID="&TP&"&DepID="&DP&"'>"&InfSubj&"</A></span>"
