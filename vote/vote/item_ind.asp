        ...
        <tr bgcolor="<%=col%>">
          ...
          <td align="center">
            <select name="ImgName" id="ImgName">
              <option value="Select" <%If ImgName="Select" Then Response.Write("selected")%>>[多选]</option>
              <option value="SelSin" <%If ImgName="SelSin" Then Response.Write("selected")%>>[单选]</option>
              <option value="FBlank" <%If ImgName="FBlank" Then Response.Write("selected")%>>必填空</option>
              <option value="GBlank" <%If ImgName="GBlank" Then Response.Write("selected")%>>可选空</option>
            </select>          </td>
...
%>
        ...
        <form action="?" method="post" name="fm01">
          <tr>
            ...
            <td align="center" bgcolor="#FFFFFF"><select name="ImgName" id="ImgName">
              ...
              <option value="SelSin" >[单选]</option>
              <option value="FBlank" >填空</option>
              <option value="GBlank" >可选空</option>
            </select></td>
    ...
    <tr bgcolor="#FFFFFF">
      <td colspan="6" align="left" bgcolor="#FFFFFF"> &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;<span class="colF0F">注意</span>: 先增加调查项，再编辑具体选项，<span class="col00F">具体选项之间用</span><span class="colF0F"> | </span><span class="col00F">分开</span>； <span class="col00F">先设置选择，后设置填空(问答)</span>，否则统计结果出错；一旦调查投入使用，<span class="col00F">任何增加调查选项或删除调查选项或修改固定顺序的操作都影响调查结果</span>，甚至出错（可以修改选项，可从最后增加填空是可以的，但不推荐这样）！</td>
    </tr>
        </form>
      </table>
      </FIELDSET></td>
