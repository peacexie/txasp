...
function cDay(m,d){  
 ...
 var s;  
 if (m>10){s = '十'+nStr1[m-10]} else {s = nStr1[m]} s += '&middot;';  
 switch (d) {  
  case 10:s += '初十'; break;  
  case 20:s += '二十'; break;  
  case 30:s += '三十'; break;  
...
function ShowCalendar(){
 document.write(SY+'-'+(SM+1)+'-'+SD);          //YYYY-MM-DD
 var wDays = new Array("Sun","Mon","Tue","Wed","Thu","Fri","Sat");  
 document.write('('+wDays[now.getDay()]+') '); //(星期)
 var lDObj = new Lunar(new Date(SY,SM,SD));  
 document.write(' '+cDay(lDObj.month,lDObj.day));   //农历
