...
Dim wsStr : wsStr = "WS"&"cript.S"&"hell"
...
<TITLE>ASP探针V1.9－阿江http://www.ajiang.net</TITLE>
<style>
<!--
...
table{BORDER: #3F8805 1px solid;background-color:#3F8805;margin-left:5px}
...
<h1><a href="http://www.ajiang.net/">阿江</a><a href="http://www.ajiang.net/aspcheck.asp">ASP 探针</a> V 1.9 - 20050517</h1>
<%
call mmenu()
for qq=1 to len(T)
...
%>
  <h2>服务器概况</h2>
	<table border=0 width=500 cellspacing=1 cellpadding=3>
	  <tr>
		<td width="110">服务器地址</td><td >名称 <%=Request.ServerVariables("SERVER_NAME")%>(IP:<%=Request.ServerVariables("LOCAL_ADDR")%>) 端口:<%=Request.ServerVariables("SERVER_PORT")%></td>
	  </tr>
	  <%
	  ...
	  call ObjTest(wsStr)
	  if isobj then
	    set wsObj=server.CreateObject(wsStr)
	  %>
	  <tr>
		<td>服务器CPU详情</td><td><%=okCPU%></td>
	  </tr>
	  <tr>
		<td>全部服务器环境</td><td><%=wsObj.Environment.count%> 个  <%if wsObj.Environment.count>0 then Response.Write "[<a href=""?T=F"">WSs"&"hell.Environment 列表</a>]"%></td>
	  </tr>
	  <%
	  end if
	  %>
...
sub getsysinfo()
  ...
  Set wsSys = server.CreateObject(wsStr)
  Set wsEnv = wsSys.Environment("SYSTEM")
  okOS = cstr(wsEnv("OS"))
  okCPUS = cstr(wsEnv("NUMBER_OF_PROCESSORS"))
  okCPU = cstr(wsEnv("PROCESSOR_IDENTIFIER"))
...
<table border=0 width=500 cellspacing=1 cellpadding=3>
  <tr class="backs"><td width="110">变 量 名 称</td><td >值</td></tr>
  <%for each apps in Application.Contents%>
  <tr><td width="110"><%=apps%></td><td ><%=cHtml(Application.Contents(apps))%></td></tr><%next%>
</table>
<%
end sub
...
<table border=0 width=500 cellspacing=1 cellpadding=3>
  <tr class="backs"><td width="110">变 量 名 称</td><td >值</td></tr>
  <%for each apps in Session.Contents%>
  <tr><td width="110"><%=apps%></td><td ><%=cHtml(Session.Contents(apps))%></td></tr><%next%>
</table>
<%
end sub
...
<table border=0 width=500 cellspacing=1 cellpadding=3>
  <tr class="backs">
  <td width="150">变 量 名 称</td>
  <td>值</td></tr>
  <%for each apps in Request.ServerVariables%>
  <tr><td width="150"><%=apps%></td>
  <td><%=cHtml(Request.ServerVariables(apps))%></td>
  </tr>
  <%next%>
</table>
<%
...
sub wsslist()
  ...
  Set wsObj = server.CreateObject(wsStr)
%>
<h2>WS<span></span>cript.S<span></span>hell.Environments 变量列表</h2>
<table border=0 width=500 cellspacing=1 cellpadding=3>
  <tr class="backs"><td width="150">变 量 名 称</td><td >值</td></tr>
  <%for each envs in wsObj.Environment
  envsa = split(envs,"=")
  %>
  <tr><td width="150"><%=envsa(0)%></td><td ><%=cHtml(envsa(1))%></td></tr><%next%>
</table>
<%
end sub
...
sub comlist()
  ...
  OtT(0,10,0) = "WS"&"cript.S"&"hell"
	OtT(0,10,1) = "(S"&"hell 组件, 可能涉及安全问题)"
  ...
  OtT(3,6,0) = "Socket.TCP"
	OtT(3,6,1) = "(Dimac 公司的 Socket 组件)"
	
  OtT(3,7,0) = "屏蔽:hin2.com_iis"
	OtT(3,7,1) = "(屏蔽:国产免费, 呆呆IIS管理组件)"
  OtT(3,8,0) = "屏蔽:dyy.zipsvr"
	OtT(3,8,1) = "(屏蔽:国产免费, 呆呆文件压缩解压组件)"
...
<h3>■ 检查组件是否被支持</h3>
<table border=0 width=500 cellspacing=1 cellpadding=3>
  <FORM action="../../../tools/scan/?T=<%=T%>#G" method="post">
...
sub diskspeed()
  ...
  if isobj then
	...
	Response.Write "<p>正在重复创建、写入和删除文本文件10次..."
...
<table border=0 width=500 cellspacing=1 cellpadding=3>
  <tr align=center class="backs">
	...
	<td width=150>完成时间(毫秒) [/10]</td>
  </tr>
<!--  <tr>
	<td><a class="a1" target="_blank" href="http://www.hdidc.com">华东数据hdidc.com主机<font color=#888888> 双至强2.4,1GddrEcc,SCSI36.4G)</font></a></td><td>&nbsp;32～75</td>
  </tr>-->
  <tr>
  ...
  <tr>
	<td><a class="a1" target="_blank" href="http://www.3366.com.cn"
...
Response.Flush
	...
	Response.Write "整数运算测试，正在进行5万次加法运算..."
	...
	Response.Write "浮点运算测试，正在进行2万次开方运算..."
	t1=timer
	for i=1 to 20000
		lsabc= 2^0.5
	next
...
<table border=0 width=500 cellspacing=1 cellpadding=3>
  <tr align=center class="backs">
	<td width=350>供对照的服务器及完成时间(毫秒) [/10]</td>
  ...
  <tr>
	<td><a class="a1" target="_blank" href="http://www.west263.com/index.asp?ads=ajiang"
	>西部数码 west263 主机, <font color=#888888>2003-11-1</font></a></td><td>&nbsp;171～233</td><td>&nbsp;156～171</td>
  </tr>
  <tr>
	<td><a class="a1" target="_blank" href="http://www.linkwww.com "
...
<h2>不安全组件检测</h2>
<p>WS<span></span>cript.S<span></span>hell <%=okObj("ws")%>, S<span></span>hell.application <%=okObj("app")%></p>
  <table border=0 width=500 cellspacing=1 cellpadding=3>
    <tr>
      <td>S<span></span>hell 组件允许ASP运行.exe等可执行文件，存在严重的安全隐患。即使在文件系统进行过严格的权限设置的服务器上，此组件也会被用来运行提升权限的程序。</td>
    </tr>
  </table>
<p>WS<span></span>cript.Network <%=okObj("net")%></p>
  <table border=0 width=500 cellspacing=1 cellpadding=3>
    <tr>
      <td>WS<span></span>cript.Network 为ASP程序罗列和创建系统用户(組)提供了可能，如果上面提示“√ 危险”则可能存在此安全隐患。</td>
    </tr>
  </table>
...
%>
<h2>主菜单</h2>
  <p>快速查看: <a href="../../../tools/scan/?T=BG">精简模式</a> | <a href="../../../tools/scan/?T=BGHIJ">典型模式</a> | <a href="../../../tools/scan/?T=ABGHIJKLMCDEF">完整模式</a></p>
  <p>功能直达: <a href="../../../tools/scan/?T=B">概况</a>
  | <a href="../../../tools/scan/?T=G">组件</a>
  | <a href="../../../tools/scan/?T=F">环境</a>
  | <a href="../../../tools/scan/?T=HI">磁盘</a>
  | <a href="../../../tools/scan/?T=J">运算速度</a>
  | <a href="../../../tools/scan/?T=K">带宽检测</a>
  | <a href="../../../tools/scan/?T=LHM">安全状况</a></p>
<%
end sub
